...
export const imageMessageController = async (req, res) => {
  try {
    ...
    const generatedImageUrl = `${process.env.IMAGEKIT_URL_ENDPOINT}/ik-genimg-prompt-${encodedPrompt}/quickgpt/
    ${Date.now()}.png?tr=w-800,h-800`;

    // Trigger generation by fetching from imagekit
    const aiImageResponse = await axios.get(generatedImageUrl, { responseType: "arraybuffer" })

    console.log("AI Image Response Status:", aiImageResponse.status);
    console.log("AI Image Response Data Length:", aiImageResponse.data.byteLength);
